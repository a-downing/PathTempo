...
The core timing model uses scalar path position, velocity, acceleration, and jerk. A path piece supplies its length, programmed velocity, and ordered differential constraint stations containing the arc-length derivatives `q'`, `q''`, and `q'''`. Lines and arcs normally produce one path piece. Every non-empty cubic or quintic spline knot interval produces one path piece.
