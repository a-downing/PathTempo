...
`PathPlanner` solves ordered pieces with tangent- and curvature-continuous boundaries. It applies each piece's maximum velocity plus aggregate and per-coordinate limits, constructs internal boundary velocities with jerk-limited forward/backward reachability, and materializes every piece through Ruckig. Internal piece boundaries use zero scalar acceleration; the requested beginning and ending accelerations are preserved. Curved candidates are checked at every supplied differential station using complete coupled acceleration and jerk. Violating pieces receive a bounded local time-scale correction and are re-solved. The result reports the effective velocity, acceleration, and jerk limits used for each piece after those corrections.
